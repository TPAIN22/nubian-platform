...
- The default Nubian markup is a single env knob, never a literal. Backend `NUBIAN_MARKUP` (`src/lib/pricing.config.js`) is authoritative; the dashboard (`NEXT_PUBLIC_NUBIAN_MARKUP`, `src/lib/pricing.config.ts`) and mobile (`EXPO_PUBLIC_NUBIAN_MARKUP`, `constants/pricing.ts`) mirror it for form defaults and un-enriched-payload fallbacks only. Import `DEFAULT_NUBIAN_MARKUP` instead of writing the number. Changing the env var affects new/fallback values only — run `node src/scripts/migrate_nubian_markup.js` to rewrite existing products.
